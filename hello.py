...
def loginDetails():
    ...
    try:
        db = pymysql.connect(host=ip, user=username, password=password)
        if (db):
            global result
...
def query_process():
    ...
    try:
        ...
        print(request.form['query_button'] == "gener_bt")
        if request.form['query_button'] == "gener_bt":
            ...
            print("clause details:", clauses)
            [sql_query, query_type] = clauses.create_query()
            print("query created:", sql_query)
            if query_type == "S":
                try:
